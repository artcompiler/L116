...
_share.messages[1003] = "No aync callback provided.";
...
var translate = function () {
  ...
  function visit(nid, options, resume) {
    ...
    (0, _share.assert)(typeof fns[node.tag] === "function", (0, _share.message)(1004, [JSON.stringify(node.tag)]));
  ...
  function target(node, options, resume) {
    visit(node.elts[0], options, function (err1, val1) {
      visit(node.elts[1], options, function (err2, val2) {
        if (val2.type !== "a") {
          err2.concat(error("Expected the second argument to be the result of 'href'", node.elts[1]));
          val2.attrs = {};
        }
  ...
  function form(node, options, resume) {
    visit(node.elts[0], options, function (err1, val1) {
      var id = val1.value;
      var src;
      var ids = (0, _share.decodeID)(id);
      if (ids[1] !== 0) {
        // It's and ID.
        src = "/form?id=" + (0, _share.encodeID)(ids);
      } else {
  ...
  function snap(node, options, resume) {
    visit(node.elts[0], options, function (err1, val1) {
      ...
      if (ids[1] !== 0) {
        // It's and ID.
        src = "https://acx.ac/snap?id=" + (0, _share.encodeID)(ids);
      } else {
        // It's a url, so request it and extract the ID.
        src = val1.value;
      }
